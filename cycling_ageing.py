...
# fmodel.save('negativemodelD.fokl')
# #
# positive_j0_model = FoKLRoutines.load('positive_j0.fokl')
# n = 100
...
#
# negative_j0_model = FoKLRoutines.load('negative_j0.fokl')
# n = 100
# x1 = np.linspace(0,35839,n)
...
def pj0(c_e, c_s_surf, c_s_max, T):
    fmodel = FoKLRoutines.load('positive_j0_model.fokl')
    n = 100
...
def nj0(c_e, c_s_surf, c_s_max, T):
    fmodel = FoKLRoutines.load('negative_j0_model.fokl')
...
sim1.solve(solver=pb.CasadiSolver(mode="fast with events"))
sim2 = pb.Simulation(model, experiment=experiment, parameter_values=param2)
sim2.solve(solver=pb.CasadiSolver(mode="fast with events"))
sims = [sim1, sim2]
