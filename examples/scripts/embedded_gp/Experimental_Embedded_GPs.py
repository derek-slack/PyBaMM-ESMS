...
class Embedded_GP_Model:
    ...
    def neg_log_likelihood(self, betas, d=False):
        ...
        if d:
            ...
            results = self.equation(betas_list, self.discmtx)
        else:
            betas_list = self.GP_Processing(betas=betas)
            self.betas = betas
            results = self.equation(betas_list,self.discmtx)
        ...
        neg_log_likelihood = 0.5 * np.log(2 * np.pi * np.exp(variance)) + (error ** 2 / (2 * np.exp(variance)))
        ...
        if not d:
            self.neg_log_likelihood_i = np.sum(neg_log_likelihood) + neg_log_prior
    ...
    def d_neg_log_likelihood_create(self):
        ...
        def numerical_grad_parallel(betas, h=1e-5):
            betas = jnp.array(betas)
            def compute_partial_gradient(betas, i):
                t_betas = betas.copy()
                t_betas = t_betas.at[i].set(t_betas[i] + h * t_betas[i])
                gi = (self.neg_log_likelihood(t_betas, d=True) - self.neg_log_likelihood_i) / (h * t_betas[i])
                return gi

            indices = jnp.arange(len(betas))  # Create an array of indices for each beta

            # Vectorize over indices using jax.vmap, since pmap is meant for multi-device parallelism
            compute_grad_vmap = jax.vmap(compute_partial_gradient, in_axes=(None, 0))

            grad = compute_grad_vmap(betas, indices)  # Compute gradients for all indices in parallel
            return grad
